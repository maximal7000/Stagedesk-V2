      ...
      actions={canEdit && (
        <div className="flex gap-2">
          ...
          <Link to={`/ausleihen?veranstaltung=${eventId}`}
            className="inline-flex items-center gap-1 px-3 py-1.5 bg-accent hover:bg-accent-hover text-white rounded-lg text-sm">
          ...
          {ausleihlisten.map((a) => (
            <div key={a.id} className="flex items-center justify-between py-3 px-4 bg-gray-800 rounded-lg">
              <div className="flex-1 min-w-0">
                <Link to={`/ausleihen/${a.id}`} className="text-white hover:text-accent font-medium flex items-center gap-2">
                ...
                <div className="flex flex-wrap items-center gap-3 mt-1 text-sm text-gray-400">
                  ...
                  <span className={`inline-flex px-1.5 py-0.5 rounded text-xs font-medium ${
                    a.status === 'aktiv' ? 'bg-accent/15 text-accent' :
